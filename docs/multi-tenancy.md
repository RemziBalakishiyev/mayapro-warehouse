...
**✅ BE#46 — format sızığı da bağlandı.** Yuxarıdakı qlobal yoxlama telefonu **sətir kimi** müqayisə edirdi, ona görə `0501234567` və `+994 50 123 45 67` bir-birindən azad görünürdü: eyni nömrə iki mağazada qeydiyyatdan keçə bilirdi və 4-cü bənd yenidən işə düşürdü. İndi həm yoxlama, həm login lookup-u, həm də yazılan dəyər **kanonik** formadadır (`994XXXXXXXXX`, `PhoneNormalizer`), yəni müqayisə nömrənin özünü müqayisə edir, onun yazılışını yox. `IX_Users_TenantId_Phone` də artıq kanonik dəyərləri qoruyur. Login-də oxuna bilməyən nömrə format xətası deyil, eyni neytral `Auth.InvalidCredentials` alır — əks halda cavabların fərqi özü informasiya olardı. Mövcud sətirlər `NormalizePhoneNumbers` miqrasiyaları ilə gətirildi; miqrasiya bu birləşmədən doğan hər hansı dublikatı yazmazdan ƏVVƏL aşkarlayıb `THROW` ilə dayanır (`DATABASE.md`).
