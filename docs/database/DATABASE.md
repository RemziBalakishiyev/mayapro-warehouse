...
- **Miqrasiya nəticə loqu → `ILogger` (BE#50)**: `RAISERROR(..., 0, 1) WITH NOWAIT` mesajı yalnız `SqlConnection.InfoMessage` hadisəsinə çatır — EF Core onu `ILogger`-ə ötürmür. Startup axınında (`ModuleExtensions.MigrateModuleWithRetryAsync`) hər modulun miqrasiyası işə düşməzdən əvvəl scope-un paylaşılan bağlantısı (`IDbConnectionFactory`) `InfoMessage`-ə abunə olunur və hər sətir tətbiqin öz `ILogger`-inə ötürülür; miqrasiya bitən kimi (uğurlu və ya uğursuz, hər ikisində) abunə dərhal ləğv olunur — bağlantı scope ilə bağlansa belə, handler-in özü ondan əvvəl götürülür, event handler leak yoxdur. Səviyyə mesajın məzmunundan asılıdır: `cevrile bilmedi: M` , M > 0 olduqda `Warning`, əks halda (M = 0 və ya marker heç yoxdursa) `Information`. Beləliklə `[BE#46] <schema>.<cədvəl>.Phone - normallasdirildi: N, cevrile bilmedi: M` sətri artıq tətbiqin standart log çıxışında (konsol/fayl, Serilog vasitəsilə) görünür — əvvəllər yalnız test helper-i (`PhoneNormalizationMigrationTests.Listening`) bu hadisəyə abunə idi, real deploy-da heç kim görmürdü. Testi: `tests/MayaPro.WarehouseApi.IntegrationTests/ModuleExtensionsMigrationLogTests.cs`.
